					...
					<div class="alert alert-info">- توسط این بخش می توانید نسبت به پرداخت قبوض آب، برق، گاز، تلفن همراه، تلفن ثابت ثابت، عوارض شهرداری، سازمان مالیات و جریمه راهنمایی و رانندگی اقدام کنید..</div>
						...
						<div class="table-responsive">
							<table class="table table-striped table-hover table-bordered" >
								<tr>
									<th>شناسه قبض</th>
									<td><input type="tel" name="bill_id" dir="auto" id="bill_id" maxlength="20" class="myform-control" value="{if isset($smarty.post.bill_id)}{$smarty.post.bill_id}{/if}" size="35" tabindex="1" required /></td>
								</tr>
								<tr>
									<th>شناسه پرداخت</th>
									<td><input type="tel" name="pay_id" dir="auto" id="pay_id" maxlength="20" class="myform-control" value="{if isset($smarty.post.pay_id)}{$smarty.post.pay_id}{/if}" size="35" tabindex="2" required /></td>
								</tr>
								<tr>
									<th>موبایل</th>
									<td><input type="tel" name="mobile" dir="ltr" id="mobile" maxlength="11" class="myform-control"  value="{if isset($smarty.post.mobile)}{$smarty.post.mobile}{/if}" size="35" tabindex="3" required /></td>
								</tr>
								<tr>
									<th></th>
									<td><button class="btn btn-primary btn-sm" type="button" onclick="check_bill();return false;" tabindex="4" ><i class="fa fa-check"></i> بررسی اطلاعات</button></td>
								</tr>
							</table>
						</div>
						{literal}
				...
				<div class="panel-body">
					...
					<div class="table-responsive">
						...
						{if isset($rows_bill)}
							...
							<tr><th width="200px" >آیدی</th><td>{$link.id}</td></tr>
							<tr><th width="200px" >شناسه قبض</th><td>{$link.bill_id}</td></tr>
							<tr><th width="200px" >شناسه پرداخت</th><td>{$link.pay_id}</td></tr>
							<tr><th>نوع قبض</th><td>{$link.bill_type|bill_type}</td></tr>
							<tr><th>مبلغ قبض</th><td>{$link.amount|number_format} تومان</td></tr>
							<tr><th width="200px" >موبایل</th><td>{$link.mobile}</td></tr>
							<tr><th>نتیجه </th><td>{if $link.status eq 'paid'}<span class="btn-success btn-xs">پرداخت شده</span>{elseif $link.status eq 'unpaid'}<span class="btn-danger btn-xs">پرداخت نشده</span>{/if}</td></tr>
							<tr><th>تاریخ ایجاد</th><td>{$link.date|jdate_format}</td></tr>
							{if $link.status eq 'paid'}<tr><th>تاریخ پرداخت</th><td>{$link.pay_date|jdate_format}</td></tr>{/if}
							{if $link.status eq 'paid'}<tr><th>شماره پیگیری</th><td>{$link.refcode}</td></tr>{/if}
						{/foreach}
						{/if}
						</table>
					</div>
